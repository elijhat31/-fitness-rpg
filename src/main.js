...
const AVATARS = ['vanguard', 'trailblazer', 'striker', 'shadow', 'sentinel', 'tactician'];
...
const AVATAR_META = {
  vanguard: ['Vanguard', 'shield'], trailblazer: ['Trailblazer', 'runner'], striker: ['Striker', 'zap'], shadow: ['Shadow', 'swords'], sentinel: ['Sentinel', 'dumbbell'], tactician: ['Tactician', 'brain'],
...
const legacyAvatarEntries = [[0x1F6E1, 'vanguard'], [0x1F9D9, 'trailblazer'], [0x1F977, 'shadow'], [0x1F98A, 'striker'], [0x1F409, 'sentinel'], [0x1F916, 'tactician'], [0x1F525, 'phoenix'], [0x1F30C, 'cosmicKnight'], [0x1F4A0, 'crystalGolem'], [0x1F432, 'shadowDragon'], [0x1F338, 'nova'], [0x1F9DA, 'pulse'], [0x1F984, 'apex'], [0x1F49C, 'onyx'], [0x1F319, 'lunar'], [0x1F451, 'regal']];
const LEGACY_AVATAR_MAP = Object.fromEntries(legacyAvatarEntries.map(([code, id]) => [String.fromCodePoint(code), id]));
function avatarId(value) { return LEGACY_AVATAR_MAP[value] || value || 'vanguard'; }
function avatarCard(value, label = '') { const id = avatarId(value); const [name, iconName] = AVATAR_META[id] || AVATAR_META.vanguard; return `<div class="avatar-figure avatar-${id}"><div class="avatar-head"></div><div class="avatar-body">${icon(iconName, 'avatar-mark')}</div></div>${label ? `<span class="avatar-name">${escapeHtml(label === true ? name : label)}</span>` : ''}`; }
...
function avatarChoices(selected = AVATARS[0]) { const chosen = avatarId(selected); return [...AVATARS, ...THEME_AVATARS].map((a, i) => `<label class="avatar-choice"><input type="radio" name="avatar" value="${a}" ${chosen === a || (!chosen && i === 0) ? 'checked' : ''}/><span>${avatarCard(a, true)}</span></label>`).join(''); }
