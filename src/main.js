...
const RANKS = [
  ...
  { id: 'elite', name: 'Elite', range: 'Level 51–75', min: 51, max: 75, glow: 4 },
...
function rankProgress(s = state) { const rank = currentRankMeta(s); if (rank.id === 'mythic') return Math.min(100, Math.round(((getLevel(s.xp) - rank.min) / Math.max(1, rank.max - rank.min + 1)) * 100)); const level = getLevel(s.xp); return Math.round(((level - rank.min) / Math.max(1, rank.max - rank.min + 1)) * 100); }
