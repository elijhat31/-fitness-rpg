...
const BOSS_DAMAGE_PER_WORKOUT = 50;
const XP_PER_LEVEL = 140;
const STORAGE_KEY = 'fitness-rpg-complete-v2';
const RANKS = [
  { id: 'novice', name: 'Novice', range: 'Level 1–10', min: 1, max: 10, glow: 1, perks: ['Basic Gear', 'Foundational Stats', 'New to the Path'] },
  { id: 'adventurer', name: 'Adventurer', range: 'Level 11–25', min: 11, max: 25, glow: 2, perks: ['Improved Gear', 'Bonus Attributes', 'New Abilities'] },
  { id: 'champion', name: 'Champion', range: 'Level 26–50', min: 26, max: 50, glow: 3, perks: ['Advanced Gear', 'Increased Stats', 'Special Skills'] },
  { id: 'veteran', name: 'Veteran', range: 'Level 51–75', min: 51, max: 75, glow: 4, perks: ['Epic Gear', 'Stronger Bonuses', 'Elite Abilities'] },
  { id: 'mythic', name: 'Mythic', range: 'Level 76–100', min: 76, max: 100, glow: 5, perks: ['Legendary Gear', 'Massive Bonuses', 'Signature Skills'] },
  { id: 'legend', name: 'Legend', range: 'Level 100+', min: 101, max: Infinity, glow: 6, perks: ['Mythical Gear', 'Max Potential', 'Become a Legend'] },
];
...
const GOALS = {
  ...
  legsCore: { label: 'Female Legs + Core Mode', focus: 'Glutes + Core', icon: 'sparkles' },
...
const PLAN_LIBRARY = {
  ...
  legsCore: {
    Monday: ['Glute Power + Planks', 'Strength', 'soft-rpg', [['Hip Thrusts', '5', '8–10'], ['Bulgarian Split Squats', '4', '8 each'], ['Glute Bridges', '4', '12–15'], ['Planks', '4', '60 seconds']]],
    Tuesday: ['Core Sculpt Circuit', 'Endurance', 'soft-rpg', [['Russian Twists', '4', '24'], ['Leg Raises', '4', '15'], ['Bicycle Crunches', '4', '30'], ['Stair Climber', '1', '20 minutes']]],
    Wednesday: ['Unilateral Leg Quest', 'Strength', 'soft-rpg', [['Walking Lunges', '4', '24 steps'], ['Bulgarian Split Squats', '3', '10 each'], ['Hip Thrusts', '4', '10–12'], ['Planks', '3', '75 seconds']]],
    Thursday: ['Stair Climber Conditioning', 'Endurance', 'soft-rpg', [['Stair Climber', '1', '30 minutes'], ['Glute Bridges', '4', '15'], ['Russian Twists', '3', '30'], ['Leg Raises', '3', '18']]],
    Friday: ['Glute Hypertrophy', 'Strength', 'soft-rpg', [['Hip Thrusts', '4', '10–12'], ['Walking Lunges', '4', '20 steps'], ['Glute Bridges', '3', '20'], ['Bicycle Crunches', '4', '30']]],
    Saturday: ['Legs + Core Gauntlet', 'Endurance', 'soft-rpg', [['Bulgarian Split Squats', '4', '10 each'], ['Walking Lunges', '4', '30 steps'], ['Stair Climber', '1', '25 minutes'], ['Planks', '4', '60 seconds']]],
    Sunday: ['Recovery Core Flow', 'Mobility', 'soft-rpg', [['Glute Bridges', '3', '15'], ['Planks', '3', '45 seconds'], ['Russian Twists', '3', '20'], ['Easy Stair Climber', '1', '12 minutes']]],
  },
  general: {
...
function generateWorkoutPlan(profile = state.profile) {
  ...
  return Object.fromEntries(DAYS.map((day) => {
    ...
    return [day, { title, focus, exercises, bossDamage: BOSS_DAMAGE_PER_WORKOUT, theme }];
  }));
}
...
function normalizeHearts(hearts = {}) {
  const updatedAt = Number(hearts.updatedAt || Date.now());
  const current = Math.max(0, Math.min(MAX_HEARTS, Number(hearts.current ?? MAX_HEARTS)));
  const gained = Math.floor((Date.now() - updatedAt) / HEART_REGEN_MS);
  if (gained <= 0 || current >= MAX_HEARTS) return { current, updatedAt };
  return { current: Math.min(MAX_HEARTS, current + gained), updatedAt: current + gained >= MAX_HEARTS ? Date.now() : updatedAt + gained * HEART_REGEN_MS };
}
function refreshHearts() { state.hearts = normalizeHearts(state.hearts); return state.hearts; }
function heartsDisplay() { const hearts = refreshHearts(); const next = hearts.current >= MAX_HEARTS ? 'Full hearts' : `Next heart in ${Math.max(1, Math.ceil((HEART_REGEN_MS - (Date.now() - hearts.updatedAt)) / 3600000))}h`; return `${'❤️'.repeat(hearts.current)}${'🖤'.repeat(MAX_HEARTS - hearts.current)} • ${next}`; }
function bossKey(date = new Date()) { const start = weekStart(date); return dateKey(start); }
function currentBoss(s = state) { const key = bossKey(); if (!s.boss[key]) s.boss[key] = { hp: BOSS_MAX_HP, damage: 0, name: 'Sunday Dragon', week: key }; return s.boss[key]; }
function bossProgress() { const boss = currentBoss(); return Math.round(((BOSS_MAX_HP - boss.hp) / BOSS_MAX_HP) * 100); }
function dealBossDamage(amount = BOSS_DAMAGE_PER_WORKOUT) { const boss = currentBoss(); const previousHp = boss.hp; boss.damage += amount; boss.hp = Math.max(0, boss.hp - amount); return previousHp > 0 && boss.hp === 0; }
...
function getRankIndex(s = state) { const level = getLevel(s.xp); const index = RANKS.findIndex((rank) => level >= rank.min && level <= rank.max); return index >= 0 ? index : RANKS.length - 1; }
...
function rankProgress(s = state) { const rank = currentRankMeta(s); if (rank.id === 'legend') return 100; if (rank.id === 'mythic') return Math.min(100, Math.round(((getLevel(s.xp) - rank.min) / Math.max(1, rank.max - rank.min + 1)) * 100)); const level = getLevel(s.xp); return Math.round(((level - rank.min) / Math.max(1, rank.max - rank.min + 1)) * 100); }
...
function completeWorkout() {
  ...
  state.completedWorkouts[key] = entry; if (state.selectedDay === todayName()) state.completedWorkouts[dateKey()] = entry; if (state.selectedDay === 'Sunday' || bossDefeated) state.bossWins++; if (bossDefeated) state.rareAvatars.dragonChampion = dateKey();
  const unlocked = checkAchievements(); lastCompletion = { title: workout.title, xp: entry.xp + loot.xp + (loot.reward.xp || 0), coins: entry.coins + loot.coins + (loot.reward.coins || 0), leveled, unlocked, loot, multiplier: boost, bossDamage: workout.bossDamage || BOSS_DAMAGE_PER_WORKOUT, bossDefeated }; saveState(); render();
}
...
function RankEvolution() { return `<section class="rank-evolution"><div class="section-title"><h2>Your Journey. Your Evolution.</h2><p>Rise through the ranks. Become a legend.</p></div><div class="rank-steps">${RANKS.map((rank) => `<div class="rank-step ${rank.id === currentRankMeta().id ? 'active' : ''}"><b>${rank.name}</b><small>${rank.range}</small><div class="mini-frame rank-${rank.id}">${avatarCard(state.character.avatar, '', { gender: state.character.gender, rank })}</div><ul>${rank.perks.map((perk) => `<li>${perk}</li>`).join('')}</ul></div>`).join('')}</div></section>`; }
