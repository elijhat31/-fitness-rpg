...
function normalizeState(saved = {}) {
  const base = { onboarded: false, profile: {}, character: { name: 'Rookie', class: 'Paladin', avatar: '🛡️' }, plan: {}, selectedDay: todayName(), checked: {}, quests: {}, completedWorkouts: {}, xp: 0, coins: 0, lifetimeCoins: 0, spentCoins: 0, inventory: [], achievements: {}, bossWins: 0 };
...
function submitOnboarding(form) {
  ...
  state.character = { name: form.heroName.value.trim() || `${state.profile.name}'s Hero`, class: form.heroClass.value, avatar: form.avatar.value };
...
function completeWorkout() {
  ...
  const bossBonus = state.selectedDay === 'Sunday' ? 90 : 0; const coinBonus = state.selectedDay === 'Sunday' ? 60 : 0; const leveled = addRewards(DAILY_XP_BONUS + bossBonus, DAILY_COIN_BONUS + coinBonus);
  const entry = { id: key, date: dateKey(), day: state.selectedDay, title: workout.title, xp: DAILY_XP_BONUS + bossBonus, coins: DAILY_COIN_BONUS + coinBonus, completed: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
